...
class StockPortfolio:
    ...
    def add_stock(self, symbol, quantity):
        if symbol in self.stocks:
            ...
            self.stocks[symbol] = {'quantity': quantity, 'price': self._get_current_price(symbol)}
    ...
    def _get_current_price(self, symbol):
        # Simulate getting price from an API by returning fixed values
        if symbol == 'AAPL':
            return 150.0  # Sample price for Apple
        elif symbol == 'MSFT':
            return 300.0  # Sample price for Microsoft
        else:
            return None  # Return None for other symbols (for simplicity)
